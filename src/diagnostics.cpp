  ...
  const double & poll_rate, const std::vector<std::string> & responders_names)
{
  ...
  size_t responder_monitorings_window_size = 2 * poll_rate / responders_names.size();
  ...
  for (const std::string & responderName : responders_names) {
    ...
    auto diagnostic = std::make_unique<CheckupReliability>(
      responderName, DEFAULT_LOW_RELIABILITY_THRESHOLD, DEFAULT_HIGH_RELIABILITY_THRESHOLD);
  ...
  const double & poll_rate, const std::vector<std::string> & initiators_names)
{
  ...
  size_t initiator_monitorings_window_size = 2 * poll_rate / initiators_names.size();
  ...
  for (const std::string & initiator_name : initiators_names) {
    ...
    auto diagnostic = std::make_unique<CheckupReliability>(
      initiator_name, DEFAULT_LOW_RELIABILITY_THRESHOLD, DEFAULT_HIGH_RELIABILITY_THRESHOLD);
