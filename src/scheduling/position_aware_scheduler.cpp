...
: RTLSRoundRobinScheduler(poll_rate, initiators_names, responders_names, rangingRequestCallback),
  reachable_responders_(
    respondersPositions, researchRadius(maximalResearchDistance, initiators_positions)),
...
void RTLSPositionAwareScheduler::timer_callback_()
{
  ...
  if (selected_responders_indexes_.size() >= 2) {
    responders_poll_index_ = selected_responders_indexes_[selected_responders_poll_index_];
...
void RTLSPositionAwareScheduler::select_responders_()
{
  ...
  if (duration(now(), last_robot_position_stamp_) < durationFromSecond(1)) {
    selected_responders_indexes_ = reachable_responders_.find(last_robot_position_);
  } else {
    selected_responders_indexes_.resize(number_of_responders_);
    std::iota(selected_responders_indexes_.begin(), selected_responders_indexes_.end(), 0);
  }
}

//-----------------------------------------------------------------------------
void RTLSPositionAwareScheduler::update_robot_position(const Eigen::Vector3d & robotPosition)
{
  std::lock_guard<std::mutex> lock(mutex_);
...
//-----------------------------------------------------------------------------
const std::vector<size_t> RTLSPositionAwareScheduler::get_selected_responders_indexes()
{
...
DiagnosticReport RTLSPositionAwareScheduler::get_report()
{
  ...
  for (size_t i = 0; i < selected_responders_indexes_.size(); ++i) {
    report += diagnostics_.get_responder_report(selected_responders_indexes_[i]);
  }
