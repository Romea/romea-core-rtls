...
void expect_linear_speeds_covariance_round_trip(const Eigen::Matrix2d & expected_covariance)
{
  std::vector<unsigned char> buffer(2);
  Eigen::Matrix2d actual_covariance;

  romea::core::serialize_linear_speeds_covariance(expected_covariance, buffer.data());
  romea::core::deserialize_linear_speeds_covariance(buffer.data(), actual_covariance);

  EXPECT_NEAR(
    std::sqrt(actual_covariance(0, 0)),
    std::sqrt(expected_covariance(0, 0)),
    kVarianceStdResolution);
  EXPECT_NEAR(
    std::sqrt(actual_covariance(1, 1)),
    std::sqrt(expected_covariance(1, 1)),
    kVarianceStdResolution);
  EXPECT_DOUBLE_EQ(actual_covariance(0, 1), 0);
  EXPECT_DOUBLE_EQ(actual_covariance(1, 0), 0);
}

void expect_angular_speed_variance_round_trip(const double & expected_variance)
{
  std::vector<unsigned char> buffer(1);
  double actual_variance;

  romea::core::serialize_angular_speed_variance(expected_variance, buffer.data());
  romea::core::deserialize_angular_speed_variance(buffer.data(), actual_variance);

  EXPECT_NEAR(actual_variance, expected_variance, kAngularVarianceStdResolution);
}
...
TEST(TestTwist2DSerialization, linearSpeedsRoundTrip)
{
  ...
  romea::core::serialize_linear_speeds(Eigen::Vector2d(-10.2937, 3.2048), buffer.data());
  romea::core::deserialize_linear_speeds(buffer.data(), actual_linear_speeds);
...
TEST(TestTwist2DSerialization, twist2DRoundTrip)
{
  ...
  EXPECT_NEAR(
    actual_twist.linearSpeeds.x(), expected_twist.linearSpeeds.x(), kLinearSpeedResolution);
  EXPECT_NEAR(
    actual_twist.linearSpeeds.y(), expected_twist.linearSpeeds.y(), kLinearSpeedResolution);
  EXPECT_NEAR(actual_twist.angularSpeed, expected_twist.angularSpeed, kAngularSpeedResolution);
  EXPECT_NEAR(
    std::sqrt(actual_twist.covariance(0, 0)),
    std::sqrt(expected_twist.covariance(0, 0)),
    kVarianceStdResolution);
  EXPECT_NEAR(
    std::sqrt(actual_twist.covariance(1, 1)),
    std::sqrt(expected_twist.covariance(1, 1)),
    kVarianceStdResolution);
  EXPECT_NEAR(
    std::sqrt(actual_twist.covariance(2, 2)),
