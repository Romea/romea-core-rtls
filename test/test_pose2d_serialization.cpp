...
void expect_orientation_round_trip(const double& expected_yaw) {
  ...
  EXPECT_LE(std::abs(romea::core::betweenMinusPiAndPi(actual_yaw -
                                                      expected_yaw)),
            kOrientationResolution);
}

void expect_position_covariance_round_trip(
    const Eigen::Matrix2d& expected_covariance) {
  std::vector<unsigned char> buffer(1);
  Eigen::Matrix2d actual_covariance;

  romea::core::serialize_position2d_covariance(expected_covariance,
                                               buffer.data());
  romea::core::deserialize_position2d_covariance(buffer.data(),
                                                 actual_covariance);

  const double expected_max_std =
      std::max(std::sqrt(expected_covariance(0, 0)),
               std::sqrt(expected_covariance(1, 1)));
  EXPECT_NEAR(std::sqrt(actual_covariance(0, 0)), expected_max_std,
              kPositionStdResolution);
  EXPECT_NEAR(std::sqrt(actual_covariance(1, 1)), expected_max_std,
              kPositionStdResolution);
  EXPECT_DOUBLE_EQ(actual_covariance(0, 1), 0);
  EXPECT_DOUBLE_EQ(actual_covariance(1, 0), 0);
}

void expect_orientation_variance_round_trip(
    const double& expected_variance) {
  std::vector<unsigned char> buffer(1);
  double actual_variance;

  romea::core::serialize_orientation_variance(expected_variance,
                                              buffer.data());
  romea::core::deserialize_orientation_variance(buffer.data(),
                                                actual_variance);

  EXPECT_LE(std::abs(romea::core::betweenMinusPiAndPi(actual_variance -
                                                      expected_variance)),
...
TEST(TestPose2DSerialization, positionCovarianceRejectsInvalidValues) {
  EXPECT_ANY_THROW(expect_position_covariance_round_trip(
...
TEST(TestPose2DSerialization, pose2DRoundTrip) {
  ...
  EXPECT_NEAR(actual_pose.position.x(), expected_pose.position.x(),
              kCartesianResolution);
  EXPECT_NEAR(actual_pose.position.y(), expected_pose.position.y(),
              kCartesianResolution);
  EXPECT_NEAR(actual_pose.yaw, expected_pose.yaw, kOrientationResolution);
  EXPECT_NEAR(std::sqrt(actual_pose.covariance(0, 0)), std::sqrt(0.234),
              kPositionStdResolution);
  EXPECT_NEAR(std::sqrt(actual_pose.covariance(1, 1)), std::sqrt(0.234),
              kPositionStdResolution);
  EXPECT_NEAR(std::sqrt(actual_pose.covariance(2, 2)),
